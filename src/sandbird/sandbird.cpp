...
struct sb_Stream {
  ...
  uint8_t * dataBegin;              /* File currently being sent to client */
  uint8_t * data;              /* File currently being sent to client */
  uint32_t byteSize;              /* File currently being sent to client */
...
static int mem_equal(const void *a, const void *b, size_t len) {
  const char *p = static_cast<const char *>(a), *q = static_cast<const char *>(b);
  while (len) {
    if (*p != *q) return 0;
    p++, q++, len--;
  }
...
static sb_Stream *sb_stream_new(sb_Server *srv, sb_Socket sockfd) {
  sb_Stream *st = static_cast<sb_Stream *>(malloc(sizeof(*st)));
  if (!st) return NULL;
...
static void sb_stream_close(sb_Stream *st) {
  ...
  if(st->dataBegin != nullptr) {
      free(st->dataBegin);
      st->dataBegin = nullptr;
  }
}
...
send_data:
    ...
    st->send_fp = NULL;
      ...
      int err = sb_buffer_reserve(&st->send_buf, 8192);
      if (err) return err;
      uint32_t bytesToSend = st->send_buf.cap;
      if(bytesToSend > st->byteSize)
          bytesToSend = st->byteSize;
      for(int i = 0; i< bytesToSend; i++) {
          st->send_buf.s[i] = st->data[0];
          st->data++;
      }
      st->send_buf.len = bytesToSend;
      st->byteSize -= bytesToSend;
      if (st->send_buf.len > 0) goto send_data;
...
int sb_send_file_from_byte(sb_Stream *st, const char *filename, long fromByte) {
    int err;
    char buf[32];
    size_t sz;
    FILE *fp = NULL;
    ...
    fseek(fp, 0, SEEK_END);
    if(fromByte > ftell(fp))
        return SB_EOUTOFMEM;
    sz = ftell(fp)-fromByte;
...
int sb_send_bytes(sb_Stream *st, uint8_t * data, size_t len) {
    int err = -1;
    char buf[32];
    uint8_t * dataCopy;
    size_t sz;
    ...
    /* Get file size and write headers */
    sprintf(buf, "%u", (unsigned) len);
    err = sb_send_header(st, "Content-Type", "application/octet-stream");
    if (err) goto fail;
    err = sb_send_header(st, "Content-Length", buf);
    if (err) goto fail;
    err = sb_stream_finalize_header(st);
    if (err) goto fail;
    dataCopy = (uint8_t * ) malloc(sizeof(uint8_t)*len);
    memcpy(&dataCopy[0],&data[0],len);
    st->data = dataCopy;
    st->dataBegin = dataCopy;
    st->byteSize = len;
    st->state = STATE_SENDING_FILE;
    return SB_ESUCCESS;
...
sb_Server *sb_new_server(const sb_Options *opt) {
  ...
  srv = static_cast<sb_Server *>(malloc(sizeof(*srv)));
  if (!srv) goto fail;
